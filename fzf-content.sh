...
sel=$(
  # Most-recently-active windows first (#{window_activity} = epoch of last
  # activity): sort -rn on that key, strip it, then capture each in order.
  tmux list-windows -a -F '#{window_activity} #{session_name}:#{window_index}' \
    | sort -rn | cut -d' ' -f2- | while read -r t; do
    tmux capture-pane -ep -t "$t" 2>/dev/null | awk -v t="$t" '{print t"\t"NR"\t"$0}'
  done | fzf --reverse --tiebreak=index --delimiter='	' --with-nth=3.. \
            --prompt='content> ' \
